...
public class GameManager : MonoBehaviourSingleton<GameManager>
{
    ...
    {
        TMP_Text coinsText = CoinAmount.GetComponent<TMP_Text>();
        float.TryParse(coinsText.text, out float availableCoins);
        if (availableCoins >= price)
        {
            StartCoroutine(DecreaseNumberGradually(coinsText, availableCoins,
                availableCoins - price, 2));
            return true;
        }
    ...
    IEnumerator DecreaseNumberGradually(TMP_Text coinsText, float startValue, float endValue, float totalTime)
    {
        ...
        while (currentValue > endValue)
        {
            ...
            coinsText.text = Mathf.RoundToInt(currentValue).ToString();
